...
const platformConfig: PlatformConfig = {
    adsensePerView: 0.015,     // $0.015 per view
    platformFeePercent: 0.20,  // 20% platform fee
    platformAccount: {
        bank: 'Deutsche Bank',
        holder: 'AdPot GmbH',
...
const adsensePerView = platformConfig.adsensePerView; // $0.015 per view

// Split configuration (total 100%)
const hourlyPayPerAd = platformConfig.adsensePerView * 0.30;   // 30%
const yearlyPayPerAd = platformConfig.adsensePerView * 0.30;   // 30%
const randomPayPerAd = platformConfig.adsensePerView * 0.20;   // 20%
const platformFee = platformConfig.adsensePerView * platformConfig.platformFeePercent; // 20%

let totalPlatformFees = 0;

// Replace mock users with proper tracking
let activeUsers: any[] = [];
let currentUser: { isRegistered: boolean; id?: string; balance?: number } = { isRegistered: false };
...
function simulateWatchedVideo() {
    ...
    document.getElementById('total-contributions')!.textContent = totalContributions.toString();
    ...
    if (currentUser.isRegistered) {
        ...
        document.getElementById('hourly-user-contributions')!.textContent = hourlyUserContributions.toString();
        document.getElementById('yearly-user-contributions')!.textContent = yearlyUserContributions.toString();
        document.getElementById('hourly-total-contributions')!.textContent = hourlyTotalContributions.toString();
        document.getElementById('yearly-total-contributions')!.textContent = yearlyTotalContributions.toString();
    }
    ...
    // Show income split in console
    console.log('AdSense Income Split:',
        '\nHourly Pot: $' + hourlyPayPerAd.toFixed(3),
        '\nYearly Pot: $' + yearlyPayPerAd.toFixed(3),
        '\nRandom Pot: $' + randomPayPerAd.toFixed(3),
        '\nPlatform Fee: $' + platformFee.toFixed(3),
        '\nTotal per view: $' + adsensePerView.toFixed(3),
        `\nPlatform Fee: $${platformFee.toFixed(3)} (Total: $${totalPlatformFees.toFixed(2)})`,
        `\nPlatform Account: ${platformConfig.platformAccount.iban}`
    );
}
...
function updateContributionsDisplay() {
    document.getElementById('hourly-user-contributions')!.textContent = currentUser.isRegistered ? hourlyUserContributions.toString() : '0';
    document.getElementById('yearly-user-contributions')!.textContent = currentUser.isRegistered ? yearlyUserContributions.toString() : '0';
    document.getElementById('hourly-total-contributions')!.textContent = hourlyTotalContributions.toString();
    document.getElementById('yearly-total-contributions')!.textContent = yearlyTotalContributions.toString();
}

function pickWinner(potType: string) {
    ...
    let winner: { nickname: string; balance: number }, potAmount: number;
...
function showSlotAnimation(potType: string, winner: { nickname: string; balance: number }, amount: number) {
...
function createMoneySymbol(side: string) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('click', (e) => {
        if (!loginPanel!.contains(e.target as Node) && !userIcon!.contains(e.target as Node)) {
    ...
    const amountInput = document.querySelector('.cashout-amount') as HTMLInputElement;
    ...
    function toggleLoggedIn(isLoggedIn: boolean) {
        if (isLoggedIn) {
